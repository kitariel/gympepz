    ...
    () => ({
      ...
      onDragOver({ active, over }) {
        if (over) {
          ...
          const activeData = active.data.current as SortableData | undefined;
          const overIndex = overData?.sortable?.index ?? 0;
          const activeIndex = activeData?.sortable?.index ?? 0;
      ...
      onDragCancel({ active }) {
        const activeValue = active.id.toString();
        return `Dragging was cancelled. Sortable item "${activeValue}" was dropped.`;
      },
    }),
    [value],
  );
